// server.js - Versão 9.0 (Painel Unificado, Edição Completa e Lógica de Cancelamento)
...
// LÓGICA DE ACESSO ATUALIZADA para lidar com status 'canceled'
const checkAccessAndLogin = async (req, res, customer) => {
    const now = new Date();

    const accessRule = await getAccessControlRule(customer.email);

    // 1. Verificação de Bloqueio Manual (prioridade máxima)
...
app.post("/login", loginLimiter, async (req, res) => {
    // (Esta rota permanece inalterada, pois sua lógica já delega para checkAccessAndLogin)
    ...
    try {
        ...
        } else { // (Lógica de tentativas de login e fallback para celular)
...
app.post("/eduzz/webhook", async (req, res) => {
    // (Esta rota permanece inalterada, pois sua lógica já está correta para a nova arquitetura)
...
// ===== NOVAS ROTAS DE ADMINISTRAÇÃO UNIFICADA =====

const getAdminPanelHeader = (key, activePage) => {
    // NOVO: Cabeçalho com filtros
    return `
        <style>
    ...
    try {
        ...
        html += `<h2>Clientes (${rows.length} registros)</h2>
            <table><tr>
                ...
                <th>Status Mensal</th><th>Expira em (Anual)</th><th>Status Vitalício</th>
                <th>Ações</th>
            </tr>`;
...
app.get("/admin/edit-customer", async (req, res) => {
    ...
    try {
        ...
        if (!customer.email && !accessRule.email) {
        ...
        // NOVO: Lógica para o select de Acesso Manual
        const permission = accessRule.permission;

        res.send(`
            <style>
                body { font-family: sans-serif; max-width: 600px; margin: 40px auto; }
                form div { margin-bottom: 15px; } label { display: block; margin-bottom: 5px; font-weight: bold; }
...
app.post("/admin/update-customer", async (req, res) => {
    ...
    try {
        ...
        const customerQuery = `
            INSERT INTO customers (email, name, phone, monthly_status, annual_expires_at, updated_at)
        ...
        if (permission && permission !== 'none') {
            const reason = permission === 'canceled' ? 'Acesso revogado manualmente pelo painel' : 'Acesso definido manualmente pelo painel';
            const accessQuery = `
            ...
            // Se 'Nenhum' for selecionado, remove a regra
...
// ROTA DE IMPORTAÇÃO CSV CORRIGIDA
app.get("/admin/import-from-csv", async (req, res) => {
    const { key, plan_type } = req.query;
    if (key !== process.env.ADMIN_KEY) { return res.status(403).send("<h1>Acesso Negado</h1><p>Chave de acesso inválida.</p>"); }
    if (!['anual', 'vitalicio', 'mensal'].includes(plan_type)) { return res.status(400).send("<h1>Erro</h1><p>Você precisa especificar o tipo de plano na URL.</p>"); }

    const CSV_FILE_PATH = path.join(__dirname, 'lista-clientes.csv');
    if (!fs.existsSync(CSV_FILE_PATH)) { return res.status(404).send("<h1>Erro</h1><p>Arquivo 'lista-clientes.csv' não encontrado na raiz do projeto.</p>"); }
    ...
    fs.createReadStream(CSV_FILE_PATH)
      ...
      .on('data', (row) => {
        ...
        if (email) { clientsToImport.push(row); }
      })
      .on('end', async () => {
        ...
        try {
            ...
            for (const customerData of clientsToImport) {
                ...
                if (plan_type === 'anual') {
                    // CORRIGIDO: Usa 'Data de Vencimento'
                    const expirationDateStr = customerData['Data de Vencimento'];
                    ...
                    await client.query(
                        `INSERT INTO access_control (email, permission, reason, product_id, invoice_id) VALUES ($1, 'allow', 'Importado via CSV - Vitalício', $2, $3) ON CONFLICT (email) DO NOTHING`,
                    ...
                    await client.query(
                        `INSERT INTO customers (email, name, phone, monthly_status, updated_at) VALUES ($1, $2, $3, $4, NOW())
                         ON CONFLICT (email) DO UPDATE SET name = COALESCE(EXCLUDED.name, customers.name), phone = COALESCE(EXCLUDED.phone, customers.phone), monthly_status = EXCLUDED.monthly_status, updated_at = NOW()`,
...
app.get("/app", requireLogin, async (req, res) => {
    // (A lógica desta rota já está correta e atualizada, não precisa de mudanças)
    try {
...
async function fetchWithTimeout(url, options, timeout = 90000) {
    // (Esta função auxiliar permanece inalterada)
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => { reject(new Error("Timeout: A requisição para a OpenAI demorou muito.")); }, timeout);
        fetch(url, options)
            .then(response => {
                clearTimeout(timer);
                if (!response.ok) {
...
function getPromptConfig(sermonType, duration) {
    // (Esta função de configuração de prompt permanece inalterada)
...
app.post("/api/next-step", requireLogin, async (req, res) => {
    ...
    try {
        ...
        if (step === 4) {
            ...
            if (!topic || !audience || !sermonType || !duration) { return res.status(400).json({ error: "Faltam informações para gerar o sermão." }); }
            if (!process.env.OPENAI_API_KEY) { console.error("Erro: Chave da API OpenAI não configurada."); return res.status(500).json({ error: "Erro interno: Chave da API não encontrada." }); }
            ...
            try {
                ...
                await logSermonActivity({
                    ...
                    sermon_type: sermonType, sermon_duration: duration, model_used: model, prompt_instruction: promptConfig.structure
                });
